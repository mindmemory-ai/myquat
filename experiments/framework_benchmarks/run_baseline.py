...
_PROJECT_ROOT = Path(__file__).parent.parent.parent  # myquat/
_MYQUAT_RELEASE = _PROJECT_ROOT / "target" / "release" / "examples" / "benchmark_hamiltonian"
_MYQUAT_DEBUG   = _PROJECT_ROOT / "target" / "debug" / "examples" / "benchmark_hamiltonian"
MYQUAT_BIN = _MYQUAT_RELEASE if _MYQUAT_RELEASE.exists() else _MYQUAT_DEBUG
...
def bench_myquat(terms, num_qubits, steps, evolution_time, order):
    ...
    if not MYQUAT_BIN.exists():
        return _fail("Binary not found — build with: cargo build --example benchmark_hamiltonian (or --release)")
...
def main():
    ...
    if FRAMEWORKS["myquat"]:
        ...
        print("Build with: cargo build --example benchmark_hamiltonian")
        print("       or: cargo build --release --example benchmark_hamiltonian")
